-- 환장 오디세이 자동 사냥 v2 (단일 파일)
-- 이동: Humanoid:MoveTo(체크포인트) / 걸음 22 + 달리기 48 (게임 애니메이션 자동 재생)
...
local ALL_MOBS = {
	"여우", "얼음 슬라임", "달팽이", "사슴", "곰", "토끼", "늑대",
	"돼지", "미니룡", "골렘",
	"golden_mummy", "rare_snowman", "눈 좀비", "겨울광대",
}

local Net = require(ReplicatedStorage.Shared.Net)
local AnimConfig = require(ReplicatedStorage.Shared.Defs.AnimConfig)
local attackRemote = Net.event("e89c992")
local skillRemote  = Net.event("e14ca84") -- 스킬 발사 (slot 1~4)
local potionRemote = Net.event("e039a15") -- 물약 사용
local PotionConfig = require(ReplicatedStorage.Shared.Defs.PotionConfig)

local state = {
	running = false,
	sprint = true,  -- Ctrl 항상 꾹 누른 상태 (자동 스프린트)
	range = math.huge,
	selectedMob = "전체",
	lastMobFilter = "전체",
	loopSpawned = false,
	...
	runAnimTrack = {},
	moveGoal = nil,
	autoSkill = false,       -- 오토 스킬 토글
	autoPotion = true,       -- HP 부족 시 자동 물약
...
-- ---------- 공격 애니메이션 (swing) ----------
local function loadSwing(char, animator)
	local tracks = {}
	for _, s in ipairs(AnimConfig.ATTACK_SWINGS) do
...
local function playSwing(char)
	...
	for _, t in ipairs(tracks) do
		t:Stop()
		t:Play(AnimConfig.ATTACK_FADE_IN)
	end
	task.delay(AnimConfig.ATTACK_HOLD, function()
		for _, t in ipairs(tracks) do t:Stop(AnimConfig.ATTACK_FADE_OUT) end
	end)
end

-- ---------- 몹 이름 추출 (Nameplate) ----------
local function mobName(m)
...
-- ---------- 대상 탐색 ----------
-- 이름표 라벨에서 등급/레벨 접두어를 제거 ("LV43 얼음 슬라임", "정예 LV1 토끼" → "얼음 슬라임", "토끼")
local function cleanMobName(raw)
	if not raw then return nil end
	local n = raw
	-- 반복적으로 등급/레벨 접두어를 제거 ("정예 LV1 토끼", "LV43 얼음 슬라임", "상급 늑대" → "토끼", "얼음 슬라임", "늑대")
	repeat
		local before = n
		n = n:gsub("^%s+", "")
		n = n:gsub("^[L一Vv][Vv一二三四五六七八九十0-9]+%s*", "") -- LV<숫자>
		n = n:gsub("^정예%s*", "")
		n = n:gsub("^상급%s*", "")
		n = n:gsub("^S+급%s*", "")
		n = n:gsub("^%s+", "")
...
local function mobWanted(raw)
	local name = cleanMobName(raw)
	if not name or name == "" then return false end
	if state.selectedMob == "전체" then
		for _, n in ipairs(ALL_MOBS) do
			if name == n then return true end
		end
		return false
	end
	return name == state.selectedMob
end

local function findTarget(charRoot)
...
-- ---------- 공격 ----------
local function doAttack(char, charRoot, target)
	if not target or not target.Parent then return end
	local hum = char:FindFirstChild("Humanoid")
	if not hum then return end
	local now = tick()
	if now - state.lastAttack < AnimConfig.ATTACK_COOLDOWN then return end
	playSwing(char)
...
-- ---------- 오토 스킬 ----------
local function doAutoSkill(char)
	if not state.autoSkill then return end
	local now = tick()
	if now - state.lastSkill < 2 then return end
	-- 현재 무기 스킬 슬롯 1~4 순차 발사 (서버가 쿨다운/잠금 여부를 판정)
	local fired = false
	for slot = 1, 4 do
		pcall(function() skillRemote:FireServer(slot) end)
		fired = true
	end
	if fired then state.lastSkill = now end
end

-- ---------- 오토 물약 (HP 50% 미만) ----------
local function doAutoPotion(hum)
	if not state.autoPotion then return end
	if hum.Health > hum.MaxHealth * state.potionThreshold then return end
	local now = tick()
	if now - state.lastPotion < PotionConfig.COOLDOWN then return end
	pcall(function() potionRemote:FireServer() end)
	state.lastPotion = now
end

local RUN_ANIM_ID = "rbxassetid://112661337019978"
local function playRunAnim(hum, shouldPlay)
	if not hum then return end
	local animator = hum:FindFirstChild("Animator")
	if not animator then return end
	local tp = state.runAnimTrack[hum]
	if not tp then
		local a = Instance.new("Animation")
		a.AnimationId = RUN_ANIM_ID
		local ok, t = pcall(function() return animator:LoadAnimation(a) end)
		if not ok or not t then return end
		t.Looped = true
		t.Priority = Enum.AnimationPriority.Core
		state.runAnimTrack[hum] = t
		tp = t
	end
	if shouldPlay then
		if not tp.IsPlaying then tp:Play(0.2) end
	else
		if tp.IsPlaying then tp:Stop(0.2) end
	end
end
-- ---------- 스프린트 + 달리기 애니메이션 (Running 유지 후 원하는 뛰기 애니메이션 재생) ----------
local function applySprint(hum)
	-- 오토사냥 ON 이면: Running 상태 유지(달리기 액션) + 스피드 50 + 뛰기 애니메이션 재생
	hum:SetStateEnabled(Enum.HumanoidStateType.Running, true)
	local moving = false
	if state.running then
		hum.WalkSpeed = 50
		local hrp = hum.Parent and hum.Parent:FindFirstChild("HumanoidRootPart")
		if hrp then moving = hrp.AssemblyLinearVelocity.Magnitude > 1 end
	end
	playRunAnim(hum, state.running and moving)
end

-- ---------- 메인 루프 (MoveTo 체크포인트) ----------
local MELEE = 6
local REACH_GOAL = 2.5    -- 목표 체크포인트 도달 판정
local RE_TARGET = 8       -- 목표가 이만큼 떠나면 재경로
...
-- 외부(테스트/드롭다운)에서도 시작 가능한 헬퍼
function startAutofarm()
	state.running = true
	if not state.loopSpawned then
		state.loopSpawned = true
		task.spawn(loop)
	end
end
_G.startAutofarm = startAutofarm

-- ---------- 달리기: Ctrl 자동 유지 (매번 꾹 누른 효과) ----------
-- 실제 Ctrl 키를 누를 필요 없이 항상 스프린트 상태로 유지합니다.
function setSprint(v)
	state.sprint = v and true or false
	local player = Players.LocalPlayer
	if player and player.Character then
...
local function getService(name)
	...
	if cloneref then
		local ok, r = pcall(cloneref, service)
		return ok and r or service
	end
	return service
end
...
-- 이전 실행의 남은 UI 제거 (Rayfield가 새 GUI를 다시 만들기 전에 실행)
pcall(function()
local CLEANUP_CoreGui = game:GetService("CoreGui")
for _, g in ipairs(CLEANUP_CoreGui:GetChildren()) do
	if g:IsA("ScreenGui") and (g.Name == "Rayfield" or g.Name:find("Rayfield")) then
		g:Destroy()
	end
end
local testRG = CLEANUP_CoreGui:FindFirstChild("RobloxGui")
if testRG then
	for _, g in ipairs(testRG:GetChildren()) do
		if g:IsA("ScreenGui") and (g.Name == "Rayfield" or g.Name:find("Rayfield")) then
			g:Destroy()
		end
	end
...
local Rayfield
local success, err = pcall(function()
	local cf = loadstring(EMBEDDED_RAYFIELD)
	if cf then
		pcall(function() Rayfield = cf() end)
	end
end)
if not success then
	warn("[autofarm] Rayfield 로드 실패, 기본 UI 사용:", err)
end

if Rayfield then
	local Window = Rayfield:CreateWindow({
		Name = "자동 사냥 | 환장 오디세이",
		LoadingTitle = "환장 오디세이",
		LoadingSubtitle = "자동 사냥",
		ConfigurationSaving = { Enabled = false, FileName = "autofarm" },
		Discord = { Enabled = false },
		KeySystem = false,
	})
	local Tab = Window:CreateTab("자동사냥", 0)
	Tab:CreateToggle({
		Name = "자동 사냥",
		Default = false,
		Callback = function(v)
	...
	Tab:CreateDropdown({
		Name = "사냥할 몹 (파일 내 모든 몹 선택 가능)",
		Options = opts,
		CurrentOption = "전체 (모든 몹)",
		MultipleOptions = false,
		Callback = function(o)
		o = (type(o) == "table" and o and o[1]) or o -- Rayfield는 단일 선택도 배열로 전달
		state.selectedMob = (o == "전체 (모든 몹)") and "전체" or o
